...
import { updateRequirements } from "../graphql/mutations";
export default function RequirementsUpdateForm(props) {
  const {
    id: idProp,
    requirements: requirementsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = requirementsRecord
      ? { ...initialValues, ...requirementsRecord }
      : initialValues;
    setRecruitingCompany(cleanValues.recruitingCompany);
    setRecruiterName(cleanValues.recruiterName);
    setRecruiterPhoneNumber(cleanValues.recruiterPhoneNumber);
    setRecruiterEmailAddress(cleanValues.recruiterEmailAddress);
    setRecruiterLinkedInUrl(cleanValues.recruiterLinkedInUrl);
    setClientName(cleanValues.clientName);
    setClientCareersUrl(cleanValues.clientCareersUrl);
    setTypeOfJob(cleanValues.typeOfJob);
    setClientRecruiterEmail(cleanValues.clientRecruiterEmail);
    setClientRecruiterLinkedInUrl(cleanValues.clientRecruiterLinkedInUrl);
    setClientsCity(cleanValues.clientsCity);
    setClientsState(cleanValues.clientsState);
    setClientsZip(cleanValues.clientsZip);
    setDateRequirementPosted(cleanValues.dateRequirementPosted);
    setJobTitle(cleanValues.jobTitle);
    setSkillSet(cleanValues.skillSet);
    setBillingRate(cleanValues.billingRate);
    setRequirements(cleanValues.requirements);
    setErrors({});
  };
  const [requirementsRecord, setRequirementsRecord] = React.useState(
    requirementsModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : requirementsModelProp;
      setRequirementsRecord(record);
    };
    queryData();
  }, [idProp, requirementsModelProp]);
  React.useEffect(resetStateValues, [requirementsRecord]);
  const validations = {
    recruitingCompany: [],
    recruiterName: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          recruitingCompany: recruitingCompany ?? null,
          recruiterName: recruiterName ?? null,
          recruiterPhoneNumber: recruiterPhoneNumber ?? null,
          recruiterEmailAddress: recruiterEmailAddress ?? null,
          recruiterLinkedInUrl: recruiterLinkedInUrl ?? null,
          clientName: clientName ?? null,
          clientCareersUrl: clientCareersUrl ?? null,
          typeOfJob: typeOfJob ?? null,
          clientRecruiterEmail: clientRecruiterEmail ?? null,
          clientRecruiterLinkedInUrl: clientRecruiterLinkedInUrl ?? null,
          clientsCity: clientsCity ?? null,
          clientsState: clientsState ?? null,
          clientsZip: clientsZip ?? null,
          dateRequirementPosted: dateRequirementPosted ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateRequirements,
            variables: {
              input: {
                id: requirementsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RequirementsUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || requirementsModelProp)}
